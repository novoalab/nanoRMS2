...
def basecall_and_align(fn, ref, rna, conf, MD=True, only_signal=False, only_forward=False):
    """Basecall reads from Fast5 file, align to reference and yield
    - read alignment (pysam.AlignedSegment) to the reference
    - raw read signal from Fast5 file
    - move table returned by basecaller
    - trace table returned by basecaller
    ...
    header = pysam.AlignmentHeader.from_references(faidx.references, faidx.lengths)
    # start client
    client = None
    ...
    for read_id, seq, qseq, sig, move, trace, md in basecalled_reads:
        if rna:
            seq = seq.replace("U", "T")
        # iterate hits
        primary_count = 0
        for hit in aligner.map(seq, buf=thr_buf, MD=MD): 
            ...
            if hit.is_primary:
                primary_count += 1
            # skip reverse alignments if only_forward=True
            if only_forward and hit.strand<0:
                continue
            a = mappy2sam(read_id, seq, qseq, hit, header, primary_count)
            yield a, sig, move, trace, md
...
def get_read_data(fn, only_signal=False):
    ...
    for read in f5file.get_reads():
        ...
        if only_signal:
            ...
            yield read_id, seq, qseq, sig, move, __reduce_trace(trace), md
...
def get_read_data_basecall(fn, client, _get_read_data, batch_size=100):
    ...
    for ri, read in enumerate(yield_reads(fn), 1):
        ...
        if ri%batch_size==0:
            ...
            for read_id, seq, qseq, move, trace, md in _get_read_data(client):
                yield read_id, seq, qseq, read2sig.pop(read_id), move, trace, md
...
def start_guppy_server(host, config, port, device):
    ...
    if os.path.isfile(host):
        ...
        log = open(tmp+".log", "w")
        # start guppy process
        args = [host, "-l", tmp, "-c", config, "-p", "auto"]#; print(args)
        if device: args += ["-x", device]
...
def _get_read_data_v006(client):
    ...
    for read, called in _get_completed_reads(client): 
        ...
        yield read_id, seq, qseq, move, __reduce_trace(trace), md
...
def __get_data_from_basecalled_read(read):
    ...
    move, trace = ds["movement"], ds["flipflop_trace"]
    return read_id, seq, qseq, move, __reduce_trace(trace), md
